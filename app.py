...
[data-testid="stChatMessage"] div, [data-testid="stChatMessage"] p { color: #00334d !important; }
</style>
...
def init_gemini():
    try:
        ...
        st.warning("⚠️ Gemini not active — demo mode running.")
        return None
gemini = init_gemini()
...
with tab3:
    ...
    for r in bwdb["rivers"]:
        ...
        pts=60 if color=="red" else 40 if color=="orange" else 25
        heat.extend(np.random.normal(loc=r["loc"],scale=[0.3,0.3],size=(pts,2)).tolist())
    if heat:
...
with tab4:
    ...
    if q:=st.chat_input("প্রশ্ন করুন / Ask a question..."):
        ...
        with st.chat_message("assistant"):
            if gemini:
                res=gemini.generate_content(f"You are FloodGuard AI (Bangladesh flood expert). Answer in Bangla + English (<100 words): {q}")
