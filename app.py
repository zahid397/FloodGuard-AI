...
# Custom CSS Fix (Visible Sidebar + Voice Ready)
st.markdown("""
<style>
.stApp {background:#e0f7fa!important;color:#0a192f!important;font-family:'Inter',sans-serif;}
[data-testid="stSidebar"] {
    background:linear-gradient(180deg,#01579b 0%,#0277bd 100%)!important;
    border-right:2px solid #004d99!important;
}
[data-testid="stSidebar"] * {color:white!important;font-weight:500!important;}
div[data-baseweb="select"]{background:white!important;color:#0a192f!important;border-radius:6px!important;}
div[data-baseweb="select"] div{color:#0a192f!important;}
.stButton>button{background:#0277bd!important;color:white!important;border-radius:8px!important;}
.stButton>button:hover{background:#01579b!important;}
.stSuccess,.stCaption,.stMarkdown,.stText{opacity:1!important;color:#0a192f!important;}
.weather-box{background:white;border:1px solid #81d4fa;border-radius:10px;padding:10px;font-weight:600;}
[data-testid="stChatMessage"] p{color:#0a192f!important;font-weight:500;}
[data-testid="stChatMessage"]{background:#f0f9ff;border-radius:10px;padding:8px;margin-bottom:5px;}
[data-testid="stChatInput"] textarea{background:white!important;color:#0a192f!important;border:1px solid #0277bd!important;}
...
st.caption("💻 Zahid Hasan | Gemini Flash + BWDB Mock + Voice AI Chatbot")
...
if st.sidebar.button("🔮 Predict Flood Risk", use_container_width=True):
    ...
    if gemini:
        res = gemini.generate_content(f"Flood risk prediction for {loc}. Rain {rain}, Temp {temp}, Hum {hum}, Level {level}. Give 2 Bangla + English safety tips.")
        text = res.text
...
if q := st.chat_input("Ask a question / প্রশ্ন করুন..."):
    ...
    with st.chat_message("assistant"):
        if gemini:
            reply = gemini.generate_content(f"FloodGuard AI answer (Bangla+English): {q}").text
            st.markdown(reply)
