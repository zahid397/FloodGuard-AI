...
if st.sidebar.button("🔮 Predict Flood Risk", use_container_width=True):
    ...
    try:
        if model:
            ...
            risk = {0: "Low", 1: "Medium", 2: "High"}[int(p)]
        else:
            risk = simple_predict(rain, temp, hum, level)
        st.session_state.risk = risk
    except Exception as e:
        st.session_state.risk = "Error"
...
with tab1:
    ...
    if r not in ["N/A", "Error"]:
        ...
        if r == "High":
            st.error("🚨 HIGH RISK! Evacuate low areas.")
            st.balloons()
        elif r == "Medium":
        ...
        st.info("👆 Use sidebar to predict.")
...
with tab2:
    ...
    for i, r in enumerate(bwdb["rivers"]):
        ...
        cols[i % 3].metric(f"🌊 {r['name']} Level", f"{r['level']} m",
                           delta=f"{d:+.1f} m",
...
with tab3:
    ...
    for r in bwdb["rivers"]:
        ...
        folium.Marker(r["loc"], tooltip=f"{r['name']} – {r['level']} m",
                      popup=f"<b>{r['name']}</b><br>Station:{r['station']}<br>Level:{r['level']}m<br>"
                            f"Danger:{r['danger']}m<br>Risk:{color}",
    ...
    if q := st.chat_input("প্রশ্ন করুন / Ask a question..."):
        ...
        with st.chat_message("assistant"):
            if gemini:
                ...
                st.info("Demo mode: Moderate risk in Dhaka – stay alert.")
