...
if gemini:
reply_prompt = f"FloodGuard AI reply in Bangla+English (context: {st.session_state.risk}): {q}"
reply = gemini.generate_content(reply_prompt).text
st.markdown(reply)
...
reply = f"Demo mode — Risk: {st.session_state.risk}. Example: বন্যায় উঁচু জায়গায় থাকুন / Stay on high ground."
st.markdown(reply)
...
c.drawString(50, 720, f"Location: {loc}")
c.drawString(50, 700, f"Predicted Risk: {st.session_state.risk}")
c.drawString(50, 680, f"Temperature: {t}°C, Humidity: {h}%, Rainfall: {r}mm")
if st.session_state.ai_summary:
c.drawString(50, 640, "AI Safety Tips:")
text = st.session_state.ai_summary[:250]
lines = [text[i:i+80] for i in range(0, len(text), 80)]  # Wrap text
y = 620
...
c.save()
buf.seek(0)
st.download_button("⬇️ Download PDF", buf.getvalue(), file_name="flood_report.pdf", mime="application/pdf")
